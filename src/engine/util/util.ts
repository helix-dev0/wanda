// Vendored from salinecitrine/noita-wand-simulator: src/app/util/util.ts
// Coupling fixes for this port (logic unchanged):
//   - import path '../calc/gun_enums' -> '../gun_enums' (new location under src/engine/)
//   - Preset / PresetGroup came from the app's src/app/types.ts (which drags in
//     redux + a full Wand type). Only the dead-code helpers isSinglePreset /
//     isPresetGroup reference them and nothing in the engine calls those, so the
//     two types are inlined here structurally to keep the file self-contained.
import * as GunEnumActionTypes from '../gun_enums';

// Inlined locally (see header) — structurally matches the upstream app types.
type Preset = {
  name: string;
  spells: string[];
};
